...
for filepath in filepaths:
    ...
    filename = Path(filepath).stem#从文件路径中提取文件名，不包含'.xlsx'
    ...
    df = pd.read_excel(filepath, sheet_name="Sheet 1") # read excel file
    columns= df.columns
    columns = [item.replace("_", " ").title() for item in columns]
    pdf.set_font(family="Times", size=10, style="B")
    pdf.cell(w=25, h=8, txt=columns[0], border=1)  # w 是表格宽度
    ...
    for index,row in df.iterrows():
        ...
        pdf.cell(w=25, h=8, txt=str(row["product_id"]),border=1)#w 是表格宽度
    ...
    pdf.set_font(family="Times", size=10)
    pdf.set_text_color(80, 80, 80)
    pdf.cell(w=25, h=8, txt="", border=1)  # w 是表格宽度
